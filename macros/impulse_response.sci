function impulse_response(g_closedloop)
...
//function plot impulse response of closed loop plant
...
if roots(denom(g_closedloop))<>[] then
		...
		t=0:tstep:tmax;
		resp=csim('impulse',t,g_closedloop);
		...
		messagebox(['Sorry! I cannot plot the impulse response';'Due to numerical tolerances, a pole-zero cancellation has occured.';' Please reselect point.']);
		end;
